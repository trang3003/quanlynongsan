...
<html lang="en">
  ...
  <?php
    ...
    if(isset($_GET['search']) && !empty($_GET['search'])){
      $key =$_GET['search'];
      $products=mysqli_query($conn,"SELECT *FROM `nhacungcap` WHERE `nhacungcap_ten` LIKE '%$key%'");
    }
    else{
      $products=mysqli_query($conn,"SELECT *FROM `nhacungcap`");
    }
?>
    <div id="wrapper">
    <?php require './admin_header.php'?>
      <div id="page-wrapper">
        ...
        <a href="./create_ncc.php">Thêm nhà cung cấp mới</a>
        <br>
        <br>
        <div class="row">
          <div class="col-lg-12">
            <div class="panel panel-primary">
              <div class="panel-heading">
                <h3 class="panel-title"><i class="fa fa-user"></i>  Danh sách nhà cung cấp</h3>
              </div>
              <div class="panel-body">
                  ...
                  <table class="table table-bordered table-hover table-striped tablesorter">
                    <thead>
                      ...
                      <th>STT <i class="fa fa-sort"></i></th>
                        <th>Tên nhà cung cấp <i class="fa fa-sort"></i></th>
                ...
                while($row =mysqli_fetch_array($products)){
                    ...
                    <tr>
                    <td><?=$stt?></td>
                    <td><?=$row['nhacungcap_ten']?></td>
                    <td> <?=$row['nhacungcap_diachi']?></td>
                    <td> <?=$row['nhacungcap_sdt']?></td>
                    <td><a href="./edit_ncc.php?nhacungcap_id=<?=$row['nhacungcap_id']?>">Sửa</a></td>
                    <td><a href="./delete_ncc.php?nhacungcap_id=<?=$row['nhacungcap_id']?>">Xóa</a></td>
                    </tr>
                  <?php $stt++; }?>
                    </tbody>
                  </table>
